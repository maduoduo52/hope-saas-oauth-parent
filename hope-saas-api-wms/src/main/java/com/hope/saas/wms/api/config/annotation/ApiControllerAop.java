...
public class ApiControllerAop {
    ...
    public Object around(ProceedingJoinPoint pjp, ReqAop reqAop) throws Throwable {
        ...
        CustomerInfoEntity customerInfoEntity = Constant.get().getCustomerInfoEntity();
        before(pjp, customerInfoEntity);
        ...
        if (reqAop.valiedArgs()) {
            ...
            reqData = validParamsByAnnotation(pjp, reqAop, validList, customerInfoEntity.getUserKey());
        ...
        if (reqAop.aes()) {
            if (result.getCode() == HttpStatus.HTTP_OK && !Objects.isNull(result.getData())) {
                // 返回参数加密
                result.setData(AESCodeUtil.encode(JSON.toJSONString(result.getData()), customerInfoEntity.getUserKey()));
            }
        }
        return result;
    }

    private void before(ProceedingJoinPoint pjp, CustomerInfoEntity customerInfoEntity) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        String userIpAddr = request.getRemoteAddr();
        String method = request.getRequestURI();
        String reqData = JSON.toJSONString(Arrays.toString(pjp.getArgs()));
        log.info("***用户：{}->{}，客户端的IP地址：{}, 调用方法：{}， 请求参数：{}", customerInfoEntity.getTenantId(),
                customerInfoEntity.getShipperCode(), userIpAddr, method, reqData);
    }

    private String validParamsByAnnotation(ProceedingJoinPoint pjp, ReqAop reqAop, List<String> validList, String key) {
        Object[] arguments = pjp.getArgs();
        if (Objects.isNull(arguments)) {
            throw new WmsApiException(ExceptionEnum.REQUEST_BODY_NOT_NULL.getMessage());
        }
        // 解密请求参数
        String decryValue;
        ...
        try {
            ...
            throw new WmsApiException(ExceptionEnum.PARAMS_DECODE_ERROR.getMessage());
        }
        ...
        if (object instanceof JSONObject) {
            ...
            throw new ParamsFormatException();
        }
    ...
    private void validField(Map<String, Object> paramMap, List<String> validList, Class clazz) {
        ...
        for (Field field : fields) {
            ...
            if (length != null && Objects.nonNull(o)) {
                String val = o + "";
            ...
            if (pattern != null) {
                if (StringUtils.isNotBlank(pattern.regexp()) && Objects.nonNull(o)) {
            ...
            if (formatLimit != null && Objects.nonNull(o) && StringUtils.isNotBlank(o.toString())) {
            ...
            if (decimalMin != null && Objects.nonNull(o)) {
                BigDecimal o1;
